...
interface FollowUpOfferCardProps {
  offer: FollowUpOffer;
  onStatusChange: (offerId: string, newStatus: string) => void;
  onAction: (action: string, offerId: string) => void;
}

export const FollowUpOfferCard: React.FC<FollowUpOfferCardProps> = ({
  offer,
  onStatusChange,
  onAction,
}) => {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex flex-col items-end space-y-2">
          ...
          >
            ...
            {offer.daysSinceUpdate} dana
          </span>
        </div>
      </div>

      {/* Follow-up Alert */}
      {offer.daysSinceUpdate > 7 && (
        <div className="mb-4 p-3 bg-red-50 rounded-lg border border-red-200">
          <div className="flex items-center">
            <AlertTriangle className="w-4 h-4 text-red-600 mr-2" />
            <span className="text-sm font-medium text-red-800">
              Potrebno je hitno praćenje ove ponude
            </span>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3 mb-4">
        ...
        <div className="flex items-center text-sm text-gray-500">
          ...
          <span>
            Poslednji update {new Date(offer.lastUpdated).toLocaleDateString()}{" "}
            {offer.createdBy}
          </span>
        </div>
      </div>
