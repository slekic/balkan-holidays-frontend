...
export default function Dashboard() {
  ...
  const getActionColor = (color: string) => {
    const colors = {
      ...
      green: 'border-green-300 hover:border-green-500 hover:bg-green-50 group-hover:text-green-700',
      purple: 'border-purple-300 hover:border-purple-500 hover:bg-purple-50 group-hover:text-purple-700',
      orange: 'border-orange-300 hover:border-orange-500 hover:bg-orange-50 group-hover:text-orange-700'
    };
    return colors[color as keyof typeof colors] || colors.blue;
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Dobrodošli nazad, {user?.name}</h1>
          <p className="text-gray-600 mt-1">Evo šta se dešava u vašem turističkom poslovanju danas.</p>
        </div>
            ...
            {quickActions.map((action, index) => {
              ...
              return (
                <Link
                  key={index}
                  to={action.path}
                  className={`p-4 border-2 border-dashed rounded-lg transition-colors group ${getActionColor(action.color)}`}
                >
                  <div className="flex justify-center items-center h-12 mb-3">
                    <Icon className={`w-8 h-8 ${getActionIconColor(action.color)}`} />
                  </div>
                  <p className="text-sm font-medium text-gray-700 mb-1 text-center">{action.title}</p>
